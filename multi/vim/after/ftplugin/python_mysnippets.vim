...
" Returns a parameter list of tags for a function-call doc string
" ex. ({}, {}) from PyFuncDoc(2)
function! PyFuncDoc(count)
    let st = g:snip_start_tag
    let et = g:snip_end_tag
    if a:count >= 0
        let param = st.et.repeat(', '.st.et, a:count)
    else
        let param = ''
    endif
    return '('.param.') --> '.st.et
...
\<CR>\"\"\"
\<CR>".st."fname".et.st."args:PyFuncDoc(Count(@z, ','))".et."
\<CR>".st.et."<CR>\"\"\"
\<CR>".st."pass".et."
\<CR>".st.et
exec "Snippet deflong def ".st."fname".et."(".st."args:PyCleanupArgs(@z)".et."):
\<CR>\"\"\"
\<CR>".st."fname".et.st."args:PyFuncDoc(Count(@z, ','))".et."
...
exec "Snippet % '".st."s".et."'".st."s:PyHashArgList(Count(@z, '%[^%]'))".et."<CR>".st.et
...
exec "Snippet ass assert ".st."expression".et.", '".st."output".et."'<CR>".st.et
exec "Snippet ml \"\"\"<CR>".st.et."<CR>\"\"\"<CR>".st.et
...
exec "Snippet ifn if __name__ == '__".st."main".et."__':<CR>".st.et
...
" Doc tests.
exec "Snippet doctest def _test():
\<CR>import doctest
\<CR>doctest.testmod()
\<CR>
